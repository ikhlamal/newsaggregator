...
def main():
    ...
    selected_option = st.sidebar.selectbox("Pilih Berita:", ["Berita Utama"] + [f"Berita Utama {i+1}" for i in range(len(feed.entries))])

    # Tampilkan berita yang dipilih
    if 'Berita Utama' in selected_option and feed.entries:
        if selected_option == 'Berita Utama':
        ...
        st.warning("Berita utama tidak ditemukan.")
